...
def get_tables_from_excels(table_path):
    print(f'Combining tables from {table_path}')
    ...
    for file in file_ls:
        if file.endswith('xls') or file.endswith('xlsx'):
            try:
                ...
                print(f"Cannot read table {os.path.join(table_path, file)}")
                continue
    return result
...
def split_tables(array):
    print('Cleaning and spliting tables ...')
    title_index = []
    tables = []
...
def store_clean(tables, outpath, table_name='table'):
    print(f'Storing cleaned tables to {outpath}')
